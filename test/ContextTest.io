...
EmptySpecTest := UnitTest clone do(
    ...
    test_has_no_hook_before := method(
        assertEquals(spec hook_before type, "nil")
    )
)
...
SpecWithBefore := UnitTest clone do(
    setUp := method(
        self spec := Spec describe("Spec") do(
            before(Nop)
        )
    ...
    test_has_before_message := method(
        assertEquals(spec hook_before type, "Message")
    )
)
